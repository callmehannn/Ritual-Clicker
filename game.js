...
const nodes = {
  ...
  leaderboardModeButtons: document.querySelectorAll("[data-leaderboard-mode]"),
...
function completeInvokeReward(reward) {
  ...
  recordLeaderboardEarned(reward);
  state.focus = 0;
  state.multiplier = 2;
...
nodes.leaderboardModeButtons.forEach((button) => {
  button.addEventListener("click", () => {
    leaderboardMode = button.dataset.leaderboardMode || "all";
    leaderboardPage = 0;
    renderLeaderboard();
  });
});
...
function getLeaderboardPeriodScores(progressState) {
  const periods = normalizeLeaderboardPeriods(progressState?.leaderboardPeriods);
  return {
    all: Number(progressState?.totalEarned || 0),
    weekly: periods.weekly.key === getWeekKey() ? Number(periods.weekly.earned || 0) : 0,
    daily: periods.daily.key === getDateKey() ? Number(periods.daily.earned || 0) : 0,
  };
}
    ...
    cloudLeaderboard = (data || [])
      .map((row) => {
        ...
        return {
          ...
          scores: getLeaderboardPeriodScores(progress),
          updatedAt: row.updated_at,
        };
      })
      .filter((entry) => entry && Math.max(entry.scores.all, entry.scores.weekly, entry.scores.daily) > 0);
  ...
  return [
    {
      ...
      scores: getLeaderboardPeriodScores(state),
    },
  ];
}
...
function mergeCurrentPlayerIntoLeaderboard(entries) {
  ...
  return [
    ...
    .filter((entry) => Math.max(entry.scores?.all || entry.score || 0, entry.scores?.weekly || 0, entry.scores?.daily || 0) > 0);
}

function sortLeaderboard() {
  cloudLeaderboard = cloudLeaderboard.sort((left, right) => getLeaderboardEntryScore(right) - getLeaderboardEntryScore(left));
}

function getLeaderboardEntryScore(entry) {
  return Number(entry?.scores?.[leaderboardMode] ?? entry?.score ?? 0);
}
...
function recordLeaderboardEarned(amount) {
  const earned = Number(amount) || 0;
  if (earned <= 0) return;
  state.leaderboardPeriods = normalizeLeaderboardPeriods(state.leaderboardPeriods);
  const today = getDateKey();
  const week = getWeekKey();

  if (state.leaderboardPeriods.daily.key !== today) {
    state.leaderboardPeriods.daily = { key: today, earned: 0 };
  }
  if (state.leaderboardPeriods.weekly.key !== week) {
    state.leaderboardPeriods.weekly = { key: week, earned: 0 };
  }

  state.leaderboardPeriods.daily.earned += earned;
  state.leaderboardPeriods.weekly.earned += earned;
}

function normalizeLeaderboardPeriods(periods) {
  const daily = periods?.daily || {};
  const weekly = periods?.weekly || {};
  return {
    daily: {
      key: typeof daily.key === "string" ? daily.key : getDateKey(),
      earned: Math.max(0, Number(daily.earned) || 0),
    },
    weekly: {
      key: typeof weekly.key === "string" ? weekly.key : getWeekKey(),
      earned: Math.max(0, Number(weekly.earned) || 0),
    },
  };
}

function mergeLeaderboardPeriods(left, right) {
  const first = normalizeLeaderboardPeriods(left);
  const second = normalizeLeaderboardPeriods(right);
  const today = getDateKey();
  const week = getWeekKey();
  return {
    daily: {
      key: today,
      earned: Math.max(
        first.daily.key === today ? first.daily.earned : 0,
        second.daily.key === today ? second.daily.earned : 0,
      ),
    },
    weekly: {
      key: week,
      earned: Math.max(
        first.weekly.key === week ? first.weekly.earned : 0,
        second.weekly.key === week ? second.weekly.earned : 0,
      ),
    },
  };
}
...
async function claimDailyQuestReward() {
  ...
  try {
    ...
    recordLeaderboardEarned(quest.reward);
    touchProgress();
...
async function claimOnchainDailyReward() {
  ...
  try {
    ...
    recordLeaderboardEarned(rewardNumber);
    state.ritualTestnetConnected = true;
    dailyChainStatus.address = address.toLowerCase();
    dailyChainStatus.date = getDateKey();
    dailyChainStatus.status = "checked";
...
function renderLeaderboard() {
  sortLeaderboard();
  const leaderboard = cloudLeaderboard.filter((entry) => getLeaderboardEntryScore(entry) > 0);
  const maxPage = getLeaderboardMaxPage();
    ...
    .map(
      (entry) => `
        <li class="${entry.address?.toLowerCase() === currentAddress ? "is-player" : ""}">
          ...
          <strong>${formatEssence(getLeaderboardEntryScore(entry))}</strong>
        </li>
      `,
    )
    .join("");
  ...
  nodes.leaderboardPlayerRank.innerHTML = showPlayerRank
    ? `<span>Your rank #${playerIndex + 1}</span><strong>${formatEssence(getLeaderboardEntryScore(playerEntry))}</strong>`
    : "";
...
function getLeaderboardMaxPage() {
  const visibleCount = cloudLeaderboard.filter((entry) => getLeaderboardEntryScore(entry) > 0).length;
  return Math.max(0, Math.ceil(visibleCount / leaderboardPageSize) - 1);
}
...
function defaultState() {
  return {
    ...
    dailyStreak: 0,
    dailyQuest: null,
    leaderboardPeriods: normalizeLeaderboardPeriods(),
    progressUpdatedAt: 0,
...
function normalizeState(saved) {
  ...
  const normalized = {
    ...
    leaderboardPeriods: normalizeLeaderboardPeriods(saved.leaderboardPeriods),
...
function mergeProgressStates(...states) {
  ...
  normalizedStates.forEach((next) => {
    ...
    merged.leaderboardPeriods = mergeLeaderboardPeriods(merged.leaderboardPeriods, next.leaderboardPeriods);
